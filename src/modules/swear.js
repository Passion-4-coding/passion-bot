...
const checkSwearWordsForUser = (amount, member, channel) => {
  ...
  if (totalAmount >= 3) {
    ...
    const embed = new EmbedBuilder().setColor(colors.danger).setTitle(`User has been muted`).setDescription(`${member.user.username} has been muted for ${minutes} minutes for using swear words. Please keep a positive atmosphere of communication. If you feel the mute is unfair feel free to contact administrator.`);
